...
const historical={
  ...
  research_targets_reviewed:0,
  exact_sources_materialized:0,
  note:'Historical fixture with intentionally unknown review completion.',
};

test('current public coverage validates',()=>assert.equal(validateCoverage(current).schema_version,3));
...
for (const [name,change] of [
 ...
 ['private materialization detail field',d=>{d.sources[0].materialized=1;}],
 ['legacy publication counters',d=>{d.published_signals=0;}],
 ['negative intake',d=>{d.sources[0].intake_count=-1;}],
 ['negative research targets',d=>{d.research_targets_reviewed=-1;}],
